...
def joy_cb(data):
    ...
    dt = (float(data.header.stamp.secs)+(float(data.header.stamp.nsecs/1000000000))) - previous_t
    previous_t = (float(data.header.stamp.secs)+(float(data.header.stamp.nsecs/1000000000)))
    ...
    joy_r_v_avg = sum(joy_r_v)/len(joy_r_v)
    joy_r_h_avg = sum(joy_r_h)/len(joy_r_h)
...
def point():
    ...
    AZIM_MAX_SPEED = rospy.get_param("/max_azimuth_speed", np.pi/8)
    ELEV_MAX_SPEED = rospy.get_param("/max_elev_speed", np.pi/64)
    ...
    while not rospy.is_shutdown():
        ...
        if run_var==True:
            ...
            commanded_azim += commanded_azim_speed*dt
